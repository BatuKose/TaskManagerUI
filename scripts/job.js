...
async function BütünIsleriGetirAsync(bitenGoster = true) {
    try {
        const istek = await authFetch(`http://localhost:1000/api/Job/BütünisBaslık?bitmisMi=${bitenGoster}`);
        const data = await istek.json();
        if (!istek.ok) throw new Error(data.Message || data.message || "Bilinmeyen hata");
...
function fillTableJobAll(data) {
    ...
    tablo.innerHTML = "";
    data.forEach(job => {
        const durum = statusMetni(job.status);
...
document.querySelector("#BütünIsler tbody").addEventListener("click", e => {
    ...
    if (!tr) return;
    document.querySelectorAll("#BütünIsler tbody tr").forEach(j => j.style.background = "");
...
async function IsBaslikSil() {
    try {
        ...
        const istek = await authFetch(`http://localhost:1000/api/Job/iş başlık?id=${silincekid}`, {
            method: "DELETE"
        });
        if (!istek.ok) {
            const data = await istek.json();
...
async function kendiIsleriniGetir() {
    try {
        const istek = await authFetch(`http://localhost:1000/api/Job/Kendiİslerim?userId=${gelUserId}`);
        if (!istek.ok) {
...
document.querySelector("#kendiIsler tbody").addEventListener("click", e => {
    const tr = e.target.closest("tr");
    if (!tr) return;
    document.querySelectorAll("#kendiIsler tbody tr").forEach(j => j.style.background = "");
    tr.style.background = "#ddd";
...
async function isKarsila() {
    try {
        const istek = await authFetch(`http://localhost:1000/api/Job/karsila?userId=${gelUserId}&jobId=${selectedJobId}`, {
            method: "POST",
            body: JSON.stringify({
                userId: gelUserId,
                jobId: selectedJobId
...
// =============================================
// AKTİF ÇALIŞANLAR / YÖNETİCİLER
// =============================================
async function aktifCalisanlariGetir(token) {
    try {
        const istek = await authFetch(`http://localhost:1000/api/users/calisanlar`);
        const data = await istek.json();
        if (!istek.ok) throw new Error(data.Message || data.message || "Bilinmeyen hata");
        return data;
    } catch (err) {
...
async function aktifYoneticileriGetir(token) {
    try {
        const istek = await authFetch(`http://localhost:1000/api/users/yoneticiler`);
        const data = await istek.json();
        if (!istek.ok) throw new Error(data.Message || data.message || "Bilinmeyen hata");
        return data;
    } catch (err) {
...
document.getElementById("isBaslikGuncelle").addEventListener("click", async () => {
    if (!selectedJobId) return alert("Güncellencek işi seçiniz.");
    ...
    document.getElementById("isBaslikbtnSave").addEventListener("click", async () => {
        try {
            ...
            const istek = await authFetch(`http://localhost:1000/api/Job/iş başlık?id=${selectedJobId}`, {
                method: "PUT",
                body: JSON.stringify({
                    title: text,
...
document.getElementById("isBaslikEkle").addEventListener("click", async () => {
    ...
    document.getElementById("isBaslikInsertbtnSave").addEventListener("click", async function ac() {
        ...
        try {
            const istek = await authFetch(`http://localhost:1000/api/Job/isbaslik`, {
                method: "POST",
                body: JSON.stringify({
                    title: titleText,
                    managerId: managerId,
...
function statusMetni(status) {
    const map = { 1: "Bekliyor", 2: "Karşılandı", 3: "Tamamlandı", 4: "İptal", 5: "Cezalı" };
...
async function isDetaylariniGetir() {
    try {
        ...
        const istek = await authFetch(`http://localhost:1000/api/Job/jobAllDetails?jobHeaderId=${jobId}`);
        const data = await istek.json();
...
async function isDetaySil() {
    try {
        const istek = await authFetch(`http://localhost:1000/api/Job/isdetay?jobDetayId=${selectedJobDetailId}`, {
            method: "DELETE"
        });
        if (!istek.ok) {
            const data = await istek.json();
...
async function cezaliIsler() {
    try {
        const istek = await authFetch(`http://localhost:1000/api/Job/Cezalıİsler`);
        const data = await istek.json();
        if (!istek.ok) throw new Error(data.Message || "Bilinmeyen hata");
        return data;
    } catch (err) {
...
async function isDetayiEkle() {
    ...
    try {
        const isId = selectedJobId ? selectedJobId : selectedKendiJobId;
        const istek = await authFetch(`http://localhost:1000/api/Job/iş detay`, {
            method: "POST",
            body: JSON.stringify({
                headerId: isId,
                detail: isDetay,
