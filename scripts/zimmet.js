...
async function ürünListesiGetir() {
    try {
        const res = await authFetch("http://localhost:1000/ZimmetDemirbas/ürün-listesi");
...
document.querySelector("#zimmetListesi tbody").addEventListener("click", e => {
    ...
    if (!tr) return;
    document.querySelectorAll("#zimmetListesi tbody tr").forEach(p => p.style.background = "");
...
async function KategorileriGetir(selectId = "categoriCombo") {
    try {
        const res = await authFetch("http://localhost:1000/ZimmetDemirbas/GetCategories");
        const data = await res.json();
...
async function insertCategory() {
    ...
    try {
        const res = await authFetch("http://localhost:1000/ZimmetDemirbas/InsertCategory", {
            method: "POST",
...
async function updateCategori(val) {
    ...
    const res = await authFetch(`http://localhost:1000/ZimmetDemirbas/UpdateCategory/${val}`, {
        method: "PUT",
...
async function insertProduct() {
    ...
    try {
        const istek = await authFetch("http://localhost:1000/ZimmetDemirbas/InsertProduct", {
            method: "POST",
            body: JSON.stringify({
                categoryId,
                name: urunAd,
        ...
        if (!istek.ok) throw new Error(data || "Bilinmeyen hata");
...
async function UrunSil() {
    try {
        const istek = await authFetch(`http://localhost:1000/ZimmetDemirbas/soft-delete-product/${selectedUrunId}`, {
            method: "PUT"
        });
        if (!istek.ok) {
...
async function getUsers() {
    try {
        const istek = await authFetch(`http://localhost:1000/ZimmetDemirbas/getuserforzimmet`);
        const data = await istek.json();
...
document.getElementById("btnZimmetEkle").addEventListener("click", async () => {
    ...
    document.getElementById("btnZimmetYap").addEventListener("click", async () => {
        ...
        try {
            const istek = await authFetch(`http://localhost:1000/ZimmetDemirbas/ZimmetKisiler`, {
                method: "POST",
                body: JSON.stringify({
                    userId: userId,
...
// =============================================
// DETAYLI ZİMMET LİSTESİ
// =============================================
async function DetayliZimmetListesiGetir() {
    try {
        const istek = await authFetch(`http://localhost:1000/ZimmetDemirbas/ZimmetDetayListesi`);
        const data = await istek.json();
        if (!istek.ok) throw new Error(data.Message || "Bilinmeyen hata");
        return data;
    } catch (err) {
...
async function ZimmetDetayTabloDoldur() {
    ...
    detay.forEach(d => {
        const durumMap = { 1: "Bekliyor", 2: "Onaylandı", 3: "İptal" };
        const durumUi = durumMap[d.durum] || "Tanımsız";
...
let selectedDetayId = null;

document.getElementById("detayliZimmetListesi").addEventListener("click", e => {
    const tr = e.target.closest("tr");
    if (!tr) return;
    document.querySelectorAll("#detayliZimmetListesi tbody tr").forEach(x => x.style.background = "");
    tr.style.background = "#ddd";
...
document.getElementById("excelExportBtn").addEventListener("click", async () => {
    const istek = await authFetch(`http://localhost:1000/ZimmetDemirbas/export-excel-zimmetkisiler`);
...
document.getElementById("btnZimmetIdae").addEventListener("click", () => {
    ...
    document.getElementById("btnİadeEvet").addEventListener("click", async () => {
        try {
            ...
            if (isNaN(iadeMiktar) || iadeMiktar <= 0) return alert("İade Miktarı sıfırdan büyük olmalıdır");

            const istek = await authFetch(`http://localhost:1000/ZimmetDemirbas/zimmetiade?dosyaid=${selectedDetayId}&miktar=${iadeMiktar}`, {
                method: "PATCH"
            });
            if (!istek.ok) {
                const data = await istek.json();
...
document.getElementById("btnZimmetDurum").addEventListener("click", async () => {
    ...
    document.getElementById("btnDurumEvet").addEventListener("click", async () => {
        try {
            const managerid = gelUserId; // Token'dan gelen kullanıcı ID
            const durum = parseInt(document.getElementById("durumEmum").value);

            const istek = await authFetch(`http://localhost:1000/ZimmetDemirbas/zimmetDurumDegisikligi?id=${selectedDetayId}&managerid=${managerid}&durum=${durum}`, {
                method: "PATCH"
            });
            if (!istek.ok) {
                const data = await istek.json();
